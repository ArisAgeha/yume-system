<template>
  <div :class="[$style.upload, disabled ? $style.disabled : '']" v-loading="loading && showLoading">
    ...
    <div :class="$style.file" @click="startUpload">
      <input ref="input" type="file" :accept="accept" :multiple="multiple" :disabled="loading" @change="onInputChange" />
    </div>
    <el-dialog
      width="500px"
      @close="onCropClose"
...
export default {
  ...
  data() {
    return {
      ...
      previewImageList: [],
      dealIndex: 0,
      multiCropIsEnd: false,
      supportCrop: typeof FileReader === 'function',
      filesLengthWatcher: null
    };
  },
  computed: {
    accept() {
  ...
  methods: {
    ...
    async saveCrop() {
      if (this.type === 'image' && this.crop && this.multiple && this.supportCrop) {
        if (this.multiCropIsEnd) {
          this.previewImageList.push(this.previewImage);
          this.cropLoading = true;
          let previewImageList = this.previewImageList.map((image) => {
            return image.split(',').slice(1).join(',');
          });
          await this.uploadFiles(previewImageList);
        ...
        this.cropLoading = true;
        await this.uploadFiles([this.previewImage.split(',').slice(1).join(',')]);
        this.cropLoading = false;
        this.onCropClose();
      }
    },
    ...
    async onInputChange(e) {
      ...
      if (this.maxLength !== 0 && files.length > this.maxLength) {
        this.$emit('error', new Error(`您最多只能再选择${this.maxLength}张图片`));
        return;
      }
      try {
        ...
        // 多张图片+裁剪模式
        else if (this.type === 'image' && this.crop && this.multiple && this.supportCrop) {
          /**
          *  filesLengthWatcher: 监听器，用于监听并触发当前图片的裁剪处理
          *  dealIndex: 当前处理的图片序号
          *  multiCropIsEnd: 标记当前图片是最后一张图片
          */
          this.filesLengthWatcher && this.filesLengthWatcher();
          this.dealIndex = 0;
          this.multiCropIsEnd = false;
          this.previewImageList = [];
          if (files.length === 1) this.multiCropIsEnd = true;
          this.openCropFile(files[0]);
          this.filesLengthWatcher = this.$watch('dealIndex', (newVal, oldVal) => {
            if (newVal === files.length) return;
            if (newVal === files.length - 1) this.multiCropIsEnd = true;
            if (newVal === this.dealIndex) this.openCropFile(files[newVal]);
          });
        }
        // 单张图片模式
        else {
